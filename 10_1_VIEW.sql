/* VIEW
	...
	***** VIEW 사용 시 주의사항 *****
	1) 가상의 테이블(실제 테이블 X) -> ALTER 구문 사용 불가(수정X)
...
-- 삽입 확인
SELECT * FROM V_DCOPY2; --> VIEWDP 'D0'가 삽입된  걸 확인함.
...
-- VIEW를 이용한 DML시 발생해는 문제점 == 제약조건 위배 현상
ROLLBACK;
SELECT * FROM DEPT_COPY2;
SELECT * FROM V_DCOPY2;
...
INSERT INTO V_DCOPY2 VALUES('D0','L3');
-- ORA-01400: cannot insert NULL into ("KH"."DEPT_COPY2"."DEPT_TITLE")
