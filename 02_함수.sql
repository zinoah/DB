...
SELECT MONTHS_BETWEEN(SYSDATE, '2023-07-10') "수강 기간(개월)" 
FROM DUAL;  
...
-- 0 : 숫자 한간을 의미, 여러개 작성 시 오른쪽 정렬 + 빈칸 0 추가
...
/*숫자 형변환*/
-- TO_NUMBER (문자데이터, [포맷]) : 문자형데이터를 숫자 데이터로변경 
...
-- EMPLOYEE 테이블에서
-- 급여가 500 만원 이상이면 '대'
-- 급여가 300 만원 이상 500 만 미만 이면  '중'
-- 급여가 300 만원 미만이면 '소'로 조회
