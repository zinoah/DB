...
FROM EMPLOYEE 
GROUP BY DEPT_CODE ; --> DEPT_ CODE가 같은 행 끼리 하나의 그룹이 됨
...
SUM(SALARY)  
FROM EMPLOYEE 
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭사용이 x (SELECT절 해석 X)
ORDER BY "인원 수"; --별칭 사용이 0 (SELECT절 해석 완료)
