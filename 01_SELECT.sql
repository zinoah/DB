...
--SYSDATE : 시스템의 현재시간(날짜)를 나타내는 상수 
--EX) 회원가입 시간 , 게시판 작성 시간 등등등... 많이 사용됨 
...
/*
 * 컴럼명 AS 별칭 : 별칭 띄어쓰기 X, 특수문자 X, 문자만O
 *  
...
-- 1. FROM절 : FROM 테이블명
-- 2. WHERE절 (조건절) : WHERE 컬럼명 연사자 값
-- 4. ORDER BY 컬럼명 | 별칭 | 컬럼순서 [ASC | DESC] [NULL FIRST | LAST]
--                             (오름 차순) (내림 차순)

-- EMPLOYEE 테이블에서 급여가 300만원 초과인 사원의(조건)
-- 사번, 이름, 급여, 부서코드를 조회
...
-- EMPLOYEE 테이블에서 '전'씨인 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE 
WHERE EMP_NAME LIKE '전%';
-- '_'사용 가능하지만 이름이 4글자 2글자 일수도잇당
...
-- EMPLOYEE 테이블에서 EMAIL의_ 앞에 글자가 세글자 인 사원 조회
...
-- JAVA에서 NULL : 참조하는 객체가 없을을 의미하는 값
